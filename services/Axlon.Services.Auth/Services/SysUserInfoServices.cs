...
namespace Axlon.Services.Auth.Services
{
    ...
    public class SysUserInfoServices : BaseServices<SysUserInfo>, ISysUserInfoServices
    {
        ...
        public SysUserInfoServices(IBaseRepository<SysUserInfo> repository, IBaseRepository<UserRole> userRoleRepository, IBaseRepository<Role> roleRepository) : base(repository)
        {
        ...
        public async Task<string> GetUserRoleNameStr(string openId)
        {
            string roleName = "";
            var user = (await base.Query(a => a.OpenId == openId)).FirstOrDefault();
            var roleList = await _roleRepository.Query(a => a.IsDeleted == false);
            if (user != null)
            {
                var userRoles = await _userRoleRepository.Query(ur => ur.UserId == user.Id);
        ...
        public Task<bool> AnyAsync(Expression<Func<SysUserInfo, bool>> whereExpression)
        {
            return base.Db.Queryable<SysUserInfo>().AnyAsync(whereExpression);
        }

        public async Task<string> GetUserRoleNameStr(long userId)
        {
            string roleName = "";
            var user = (await base.Query(a => a.Id == userId)).FirstOrDefault();
            var roleList = await _roleRepository.Query(a => a.IsDeleted == false);
            if (user != null)
            {
                var userRoles = await _userRoleRepository.Query(ur => ur.UserId == user.Id);
